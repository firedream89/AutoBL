...
Tache::Tache(QString version)
{
    ...
    QAction *tempsRestant = menu->addAction("Temps Restant");
    QObject::connect(tempsRestant,SIGNAL(triggered(bool)),this,SLOT(Temps_Restant()));
    QAction *Lancement = menu->addAction("Lancer l'ajout de BL");
    QObject::connect(Lancement, SIGNAL(triggered(bool)), this, SLOT(Ajout_BC()));
    QAction *Afficher = menu->addAction("Ouvrir");
...
{
    Affichage_Info("Ajout de BC en cours\r\nNe pas utiliser le pc");
}

void Tache::Arret_Ajout()
{
    Affichage_Info("Arret en cours veuillez patienter...");
